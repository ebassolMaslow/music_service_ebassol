...
<head>
    ...
    <title>Панель администратора - Добавление в альбом</title>
...
<body>
    ...
    <header>
        <div class="admin-container">
            <ul class="main-menu-panel">
                ...
                <li class="main-menu-panel_item"><a href="./adding_in_album-panel.php" class="menu-item-panel admin-menu-pressed">Добавление в альбом</a></li>
            </ul>
        </div>
        <hr>
    </header>
    ...
    <div class="div-title-admin">
        <div class="admin-container">
            <div class="add-btn-divs">
                <div class="div-title-admin">
                    <h1 class="title-admin">Добавление в альбом</h1>
                </div>
                <div class="btn_add">
                    <a href="./create_adding_in_album.php">Добавить запись</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <?php
    if (isset($_GET['ID_adding_in_album'])) {
        $AIAD = $_GET['ID_adding_in_album'];
        $qDeleteAIA = "DELETE FROM `adding_in_album` WHERE ID_adding_in_album='$AIAD'";
        addslashes($qDeleteAIA);
        $resDeleteAIA = mysqli_query($connect, $qDeleteAIA) or die(mysqli_error($connect));
    }

    $qInfoAIA = "SELECT * FROM adding_in_album, albums, track WHERE adding_in_album.ID_albums = albums.ID_albums AND adding_in_album.ID_track = track.ID_track";
    addslashes($qInfoAIA);
    $resInfoAIA = mysqli_query($connect, $qInfoAIA) or die(mysqli_error($connect));
    while ($InfoAIA = mysqli_fetch_object($resInfoAIA)) {
        echo "
    <div class=\"body-table\">
        <div class=\"admin-container\">
            <div class=\"admin_table_row\">
                <div class=\"admin_table_cell_first admin_projects_table_id\">
                    <p>$InfoAIA->ID_adding_in_album</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text margin-admin-user-table\">$InfoAIA->album_name</p>
                </div>
                <div class=\"admin_table_cell admin_projects_table_name\">
                    <p class=\"admin_table_cell-text margin-admin-user-table\">$InfoAIA->track_name</p>
                </div>
                <div class=\"admin_projects_table_act\">
                    <a href=\"edit_adding_in_album.php?idA=$InfoAIA->ID_adding_in_album\"><img class=\"admin_table_act_image\" src=\"./images/svg/edit_green.svg\"></a>
                <a href=\"?ID_adding_in_album=$InfoAIA->ID_adding_in_album\"><img class=\"admin_table_act_image\" src=\"./images/svg/delete_green.svg\"></a>
            </div>
        </div>
    </div>
";
